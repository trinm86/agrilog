...
class CaPheTongKetApp extends StatefulWidget {
  final String nam_nhap;
  const CaPheTongKetApp({super.key, required this.nam_nhap});
...
class _MyAppState extends State<CaPheTongKetApp> {
  ...
  void initState() {
    ...
    _search.text = widget.nam_nhap.isNotEmpty 
                   ? widget.nam_nhap 
                   : CurrentDate.year;
    bloc = CayTrongBloc(LoaiCay.caphe,_search.text);
    _itemsFuture = bloc.getLoiNhuans(_search.text);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        toolbarHeight: 50,
        title: const Text("Tổng kết Cà phê"),

...
class SauRiengBanApp extends StatefulWidget {
  final String search;
  const SauRiengBanApp({super.key, required this.search});
...
class _MyAppState extends State<SauRiengBanApp> {
                ...
                child: Column(
                  children: <Widget>[
                    ...
                    TextFormField(
                      ...
                      enabled: false,
                      //initialValue: integer2.format(item.thanhtien),
  ...
  void initState() {
    ...
    bloc = CayTrongBloc.withParams(LoaiCay.saurieng, LoaiNhap.ban,  _search.text);
    _itemsFuture = bloc.getAlls(_search.text);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        toolbarHeight: 50,
        title: const Text("Thu tiền Sầu riêng"),
        ...
        builder: (BuildContext context, AsyncSnapshot<List<CayTrongModel>> snapshot) {
          if (snapshot.hasData) {
            const columnWidths = {
              ...
              2: FlexColumnWidth(2.5), // Cột thứ hai chiếm 3 phần
              3: FlexColumnWidth(2), // Cột thứ ba chiếm 4 phần
              4: FlexColumnWidth(1.5), // Cột thứ ba chiếm 4 phần
              5: FlexColumnWidth(2.6), // Cột thứ ba chiếm 4 phần
            };
